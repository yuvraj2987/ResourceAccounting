...
const char* file="/usr/bin/lsof";
static char* get_process_name(char* data){
    char name[PATH_MAX];
    char* res;
    int i=0;
    if(!data)
    ...
    name[i]='\0';
    res = (char*)malloc(sizeof(char)*(strlen(name)+1) );
    if(res){
	strcpy(res,name);
    }
    return res;
...
static int patch_main(uint16_t port, int std_out_fd){
    ...
    if((fd = open("/home/pi/mmap.txt", O_RDWR|O_CREAT|O_TRUNC))==-1){
	printf("error in creating memory mapped file:%s\n",strerror(errno));
	return -1;
    }

    if((std_fd_new = dup(std_fd_old))==-1){
	printf("error in duplicating fd:%s\n",strerror(errno));
	return -1;
    }
    

    if((dup2(fd,std_fd_old)==-1)){
	printf("error in duplicating fd:%s\n",strerror(errno));
	return -1;
    }
    ...
    data = mmap((caddr_t)0, pagesize, PROT_READ|PROT_WRITE, MAP_SHARED, fd,
		0);
    
    if(data==(caddr_t *)-1){
	printf("error in  memory mapping:%s\n",strerror(errno));
	return -1;
    }
...
/*
  if((dup2(std_fd_new,std_fd_old)==-1)){
    ...
    if(pname){
	printf("\n Process %s listening to port at:%d \n",pname,port);
...
int main(int argc, char *argv[])
{
    ...
    send_to_server(REQUEST, port, &request);
		...
		if (ssock == -1)
            ...
            //printf("FARSHAD_announce: %s for client %d\n",announce,client);//FARSHAD
