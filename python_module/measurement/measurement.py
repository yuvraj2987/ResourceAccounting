...
def saveRecords_socket(records,fileName):
	with file(fileName, "a+") as fd:
		for myRecord in range(0,len(records)/3):
			myStr="%.6f %d %d" % (records[3*myRecord], records[3*myRecord+1], records[3*myRecord+2])
			fd.write(myStr+"\n")
#####
#####
class writerThread_power (threading.Thread):
	...
	def run(self):
		...
		if self.iteration%10==1:
			print "--- writingThread_power finished file writing at "+str(time.time() - self.offsetTime)
...
class writerThread_socket (threading.Thread):
	...
	def run(self):
		...
		print "--- writingThread_socket finished file writing at "+str(time.time() - self.offsetTime)
...
class measureThread (threading.Thread):
	...
	def run(self):
		...
		while True:
			time.sleep(0.01)
			rtTime = time.time() - self.offsetTime
	...
	try:
		...
		while True:
			try:	
				...
				nowTime = time.time()
				socketStr = myConnection.recv(256)
				print "--- got something form the socket: " + socketStr + " ---"
				myConnection.close()
				recordType, recordID = socketStr.split(" ",1)		
				myArray_socket.append(nowTime - startTime)
				myArray_socket.append(int(recordType))
				myArray_socket.append(int(recordID))
